...
export default function EditSubscriptionScreen({ route, navigation }: any) {
  ...
  const [startDate, setStartDate] = useState(subscription.first_payment_date);
  const [endDate, setEndDate] = useState(subscription.end_date || '');
  const [reminderDays, setReminderDays] = useState(subscription.reminder_days_before || 1);
  const [reminderRepeat, setReminderRepeat] = useState(subscription.reminder_repeat_count || 1);
  ...
  const handleUpdate = async () => {
    if (!serviceName || !price) {
      Alert.alert(t.error || 'Error', t.fillAllFields || 'Please fill out all fields.');
      return;
    }
    ...
    try {
      ...
      if (existing) {
        const isDuplicate = existing.some(sub => 
          sub.service_name.trim().toLowerCase() === trimmedName &&
  ...
  const handleDelete = async () => {
    Alert.alert(t.deleteSubscription || 'Delete Subscription', t.deleteSubscriptionConfirm || 'Are you sure you want to delete this subscription?', [
      ...
      { text: t.delete || 'Delete', style: 'destructive', onPress: async () => {
        ...
        try {
          ...
          if (error) throw error;
          Alert.alert(t.success || 'Success', t.subscriptionDeleted || 'Subscription deleted.');
          navigation.goBack();
        } catch (error: any) {
          Alert.alert(t.error || 'Error', error.message);
        } finally {
  ...
  // ... [Modals matching AddSubscriptionScreen]
          ...
          <FlatList 
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={[styles.modalItem, { borderBottomColor: colors.border }]} onPress={() => { setCategory(item); setShowCatModal(false); }}>
                <View style={[styles.colorCircle, { backgroundColor: CATEGORY_COLORS[item] || CATEGORY_COLORS.Other }]} />
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <ScrollView contentContainerStyle={styles.inner}>
            <View style={styles.header}>
              ...
              <Text style={[styles.headerTitle, { color: colors.text }]}>{t.editSubscription}</Text>
            </View>
            ...
            <View style={styles.form}>
              ...
              <TouchableOpacity style={[styles.pickerButton, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={() => setShowCatModal(true)}>
                <View style={[styles.colorCircle, { backgroundColor: CATEGORY_COLORS[category] || CATEGORY_COLORS.Other }]} />
              ...
              <TouchableOpacity style={[styles.primaryButton, { backgroundColor: colors.primary, marginTop: 16 }]} onPress={handleUpdate} disabled={loading || deleting}>
                {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>{t.update}</Text>}
              </TouchableOpacity>

              <TouchableOpacity style={[styles.dangerButton, { borderColor: colors.danger, marginTop: 16 }]} onPress={handleDelete} disabled={loading || deleting}>
                {deleting ? <ActivityIndicator color={colors.danger} /> : <Text style={[styles.dangerButtonText, { color: colors.danger }]}>{t.deleteSubscription}</Text>}
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  header: { padding: SIZES.padding, flexDirection: 'row', alignItems: 'center' },
  ...
  dangerButton: { borderWidth: 1, paddingVertical: 16, borderRadius: SIZES.radius, alignItems: 'center' },
  dangerButtonText: { fontSize: SIZES.fontMedium, fontWeight: 'bold' },
